<link type="text/css" rel="stylesheet" href="stylesheets/panel.css?<?=time()?>">


<div class="panel">
    <div class="block1 clearfix">
        <div class="block1_logo">
        </div>
        <div class="block1_leftside">
        </div>
        <div class="block1_leftside_phone">
            <h3 class="block1_content_phone">初期以「速克達」機車賽事用之排氣管為主力產品，後期研發製造高性能避震器，導入多項測試儀器以利協助開發，多年來本公司所研發生產之MD-R系列避震系統，獲得國內外廣大消費群的支持與信賴</h3>
        </div>
        <div class="block1_rightside">
            <h3 class="block1_content">初期以「速克達」機車賽事用之排氣管為主力產品，後期研發製造高性能避震器，導入多項測試儀器以利協助開發，多年來本公司所研發生產之MD-R系列避震系統，獲得國內外廣大消費群的支持與信賴</h3>
        </div>
    </div>
    <div class="block2">
        <ul class="slick_title type_select">
            <li>
                <a href="#" data-type="product1">前避震</a>
            </li>
            <li>
                <a href="#" data-type="product2">後避震</a>
            </li>
            <li>
                <a href="#" data-type="product3">重車前避震</a>
            </li>
            <li>
                <a href="#" data-type="product4">重車後避震</a>
            </li>
        </ul>
        <ul class="slick_content product1">
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <!-- <img class="product_name" src="images/UF2.png"> -->
                    
                    <p>UF2_block1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block1-1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
        </ul>
        <ul class="slick_content product2">
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block1-1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
        </ul>
        <ul class="slick_content product3">
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block3</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block3</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block1-1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
        </ul>
        <ul class="slick_content product4">
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block4</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block4</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block1-1</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2_block-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>Vespa FV-2</p>
                </a>
            </li>
            <li>
                <a href="/product_subpage">
                    <img src="slick/images/img-uf-2@2x.png">
                    <p>UF2</p>
                </a>
            </li>
        </ul>

    </div>
    <div class="block3">
        <h2>最新消息</h2>
        <ul>
            <li>
                <a href="/news_subpage">2019.09.31<span>徵人啟事</span></a>
            </li>
            <li>
                <a href="/news_subpage">2019.05.17<span>EDS電控說明書</span></a>
            </li>
            <li>
                <a href="/news_subpage">2019.09.31<span>徵人啟事</span></a>
            </li>
            <li>
                <a href="/news_subpage">2019.05.17<span>EDS電控說明書</span></a>
            </li>

            <a class="more_news" href="/news">更多消息</a>
            <a class="more_news_arrow" href="/news"></a>
                   
        </ul>
    </div>
</div>


<script type="text/javascript" src="slick/js/slick.js"></script>
<script>

$(document).ready(function(){
	var _window_width=window.innerWidth;   
	var slick_setting = {
        dots: false,
        infinite: true,
        speed: 1200,
        arrows:true,
        //autoplay:false,
        //autoplaySpeed:1200,
        cssEase: 'linear',
        slidesToShow: 4,
        slidesToScroll: 1,
        autoplay: false,
        autoplaySpeed: 2000,

        responsive: [
    
            {
                breakpoint: 530,
                settings: {
                    slidesToShow: 3,
                    slidesToScroll: 1
                }
                // breakpoint: 400,
                // settings: {
                //     slidesToShow: 2,
                //     slidesToScroll: 1
                // }
            }

        ]
    };

    $('.type_select li a').click(function(e){
        e.preventDefault();
        var el =$(this);
        var target = el.data('type');

        $('.type_select li a').removeClass('active');
        $(this).addClass('active'); 

		$('.'+target).siblings('.slick_content').each(function(i ,block){
			$(block).removeClass('active');
			if($(block).hasClass('slick-initialized')){
				$(block).slick('unslick');
			}
		});
        $('.'+target).addClass('active').slick(slick_setting);   

    });
    $('.type_select li:first-child a').click();

    
});
</script>
